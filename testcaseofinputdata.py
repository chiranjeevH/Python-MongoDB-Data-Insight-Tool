...
def mobileinput():
    ...
    # # Collection Name
    # col = db["916000"]
    # myquery = {"Number": entry.get() }
    # mydoc = col.find(myquery)
    print(myquery)
    # Fields with values as 1 will
    # only appear in the result
    output= pd.DataFrame(col.find(myquery))
    print(output)
    # if output in globals(): 
    #     Listbox.insert(output)    
...
# ---------------------------------------------------------------------------------------------------
# Defines a browse function to load a CSV
def browse():
    global file
...
def downloadCsv():
    ...
    print("File", savepath)
    csvtodownload.to_csv(savepath, index=False)
# ---------------------------------------------------------------------------------------------------
...
def nameinput():
    ...
    db = client["truecallerDB"]
    # Num=input
    dbName = str(firstEntryTabOne.get())
    # Collection Name
    col = db[dbName]
    myquery = {"Name": firstEntryTabOne.get()}
    mydoc = col.find(myquery)
    mydoc = list(mydoc)[0]    
    # # Collection Name
    # col = db["916000"]
    ...
    # Fields with values as 1 will
    # only appear in the result
    output2= pd.DataFrame(col.find(myquery))
    print(output2)
    # if output in globals(): 
    #     Listbox.insert(output2)
#-------------------------------------------------------------------------------------------------------------    
...
TrueUI_lable = Label(screen, text="Profile X", bg='#f7f7f7',
                     borderwidth=9, foreground='Black', font=("Courier", 20, "bold"))
TrueUI_lable.pack()
TrueUI_lable.place(x=410, y=0)
# === WIDGETS FOR TAB ONE
EnterName = tk.Label(tab1, text="Enter Name:",font=("Open Sans", 10, "bold"),foreground = "black",borderwidth=2)
EnterMobile = tk.Label(tab1, text="Enter Mobile:",font=("Open Sans", 10, "bold"),foreground = "black",borderwidth=2)
EnterMobile.place(x=20)




Search1 = tk.Button(tab1, text="Search",font=("Open Sans", 9, "bold"),background = "#e8e8e8",foreground = "black",borderwidth=2,command=nameinput) #,command=lambda:[mongotocsv(),csvtable()]
label4 = tk.Label(screen, text= lambda:[output(),output2()],font=('helvetica', 10, 'bold'))
Search2=  tk.Button(tab1, text="Search",font=("Open Sans", 9, "bold"),background = "#e8e8e8",foreground = "black",borderwidth=2,command=mobileinput)
...
SampleCSV = tk.Button(tab2, text="Sample CSV",font=("Open Sans", 10, "bold"),background = "#e8e8e8",foreground = "black",borderwidth=2,command=samplecsv)
Browsebutton = tk.Button(tab2, text="  Browse  ",font=("Open Sans", 10, "bold"),background = "#e8e8e8",foreground = "black",borderwidth=2,command=browse)
Download=tk.Button(tab2, text="Download",font=("Open Sans", 10, "bold"),background = "#e8e8e8",foreground = "black",borderwidth=2,command=downloadCsv)
Findbutton= tk.Button(tab2, text="    Find    ",font=("Open Sans", 10, "bold"),background = "#e8e8e8",foreground = "black",borderwidth=2, command=lambda: [bulkinput(), csvtable()])
...
Download.grid(row=5, column=2, padx=15, pady=15)   #button should be showed only when CSV table is called 🌟
